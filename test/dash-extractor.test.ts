...
import { MediaType } from '../lib/shared/media-type';
...
test('mpd extraction from text', async () => {
  ...
  const firstVideoTrack = streams.find(
    (stream) => stream.mediaType === MediaType.VIDEO,
  );
  const firstVideoSegment =
    firstVideoTrack?.playlist?.mediaParts[0]?.mediaSegments[0];
  expect(firstVideoSegment?.url).toBe(
    'https://media.axprod.net/TestVectors/v7-MultiDRM-SingleKey/1/0001.m4s',
  );

  const firstSubtitleTrack = streams.find(
    (stream) => stream.mediaType === MediaType.SUBTITLES,
  );
